...
export default function ArticlesHero() {
  ...
  return (
    <section className="relative w-full overflow-hidden" style={{ height: "60vh", minHeight: 480 }}>
      ...
      {/* ── Gradient overlays (blended with deep dark blue & black) ── */}
      ...
      {/* Deep black vignette to maximize text prominence */}
      <div className="absolute inset-0 pointer-events-none" style={{
      ...
      {/* ── CSS grid overlay (matches homepage) ── */}
      <div
      ...
      {/* ── HUD corner labels (matches homepage style) ── */}
      <motion.div
      ...
      >
        <div className="absolute top-24 left-8 hidden md:block font-mono text-[10px] tracking-[0.3em] opacity-80" style={{ color: "#E5B53A" }}>
          [ARCHIVE.ACTIVE] // INSIGHTS_ENGINE
        </div>
        <div className="absolute top-24 right-8 hidden md:block font-mono text-[10px] tracking-widest text-right" style={{ color: "rgba(255,255,255,0.35)" }}>
          TRIAL TEMPLATE<br />KNOWLEDGE_BASE
        </div>
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center text-center px-6 z-20 pt-16">
        ...
        >
          Deep dives into trial strategy, legal visualization, and the science of juror persuasion —{" "}
          <em style={{ color: "rgba(229,181,58,0.9)" }}>written by practitioners, for practitioners.</em>
        </motion.p>
        ...
        >
          {[
            { value: 6,   suffix: "+",   label: "Articles" },
            { value: 500, suffix: "+",   label: "Attorneys Served" },
