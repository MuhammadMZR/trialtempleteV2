...
export default function OurProjects() {
  ...
  // We duplicate the projects array multiple times to create a seamless infinite scroll for desktop
  ...
  return (
    <section className="bg-[#051A3D] pt-0 pb-16 overflow-hidden border-t border-white/5 relative">
      ...
      <div className="relative w-full flex overflow-hidden py-10">
        {/* Left/Right Gradient Masks for smooth fade out on desktop */}
        {!isMobile && (
          <>
        ...
        {mounted && isMobile ? (
          /* Mobile swipeable snap container */
          <div 
            className="w-full flex flex-row overflow-x-auto snap-x snap-mandatory scrollbar-none gap-6 px-6"
            style={{
          ...
          >
            {projects.map((project, i) => (
              <div 
                key={i} 
                className="w-[260px] sm:w-[300px] shrink-0 snap-center group cursor-pointer"
              >
                {/* Image Container */}
                <div className="w-full aspect-[3/4] rounded-2xl overflow-hidden mb-4 relative shadow-[0_0_30px_rgba(0,0,0,0.5)] border border-white/10">
                  <div className="absolute inset-0 bg-gradient-to-t from-[#051A3D]/80 via-transparent to-transparent opacity-80 group-hover:opacity-90 transition-opacity duration-500 z-10" />
                  <img 
                    src={project.image} 
                    alt={project.title} 
                    className="w-full h-full object-cover filter brightness-90 group-hover:brightness-110 transition-all duration-700"
                  />
                  <div className="absolute bottom-0 left-0 w-full p-6 z-20 opacity-100 transition-all duration-500">
                    <span className="text-white text-xs font-medium flex items-center gap-2">
                      View Case Study <ArrowRight className="w-3.5 h-3.5 text-gold" />
                    </span>
                  </div>
                </div>
                
                {/* Text */}
                <span className="text-gold font-mono text-[10px] tracking-widest uppercase mb-2 block opacity-85">
                  {project.category}
                </span>
          ...
          /* Desktop infinite scrolling container */
          <motion.div
